...
//コンストラクタ
//=====================================================================
MyVulkan::MyVulkan(int width, int height, GLFWwindow *window)
{
	//0.initialize member
...
//initialize window
//=====================================================================
void MyVulkan::iniWindow(int width, int height)
{
	glfwInit();
...
//=====================================================================
void MyVulkan::FrameBufferResizeCallback(GLFWwindow* window, int width, int height)
{
...
//initialize
//=====================================================================
void MyVulkan::initVulkan(int width, int height)
{
	iniWindow(width, height);
}
...
//=====================================================================
std::vector<const char*> MyVulkan::GetRequiredExtension()
{
...
//=====================================================================
VKAPI_ATTR VkBool32 VKAPI_CALL MyVulkan::DebugCallback
(
...
void MyVulkan::CreateInstance()
{
	...
	*/
	...
	appInfo.pApplicationName = "Tutorial1";
...
//1.2 set up callback
//=====================================================================
void MyVulkan::SetUpDebugCallback()
{
	if (!mEnableValidationLayers)
		return;
	//create info 構造体作成
...
//=====================================================================
VkResult MyVulkan::CreateDebugReportCallbackEXT
(
	VkInstance instance,
...
//=====================================================================
void MyVulkan::DestroyDebugReportCallbackEXT(VkInstance instance,
...
//=====================================================================
void MyVulkan::PickPhysicalDevice()
{
...
//2. check device
//=====================================================================
bool MyVulkan::isDeviceSuitable(const VkPhysicalDevice &device)
{
#if _DEBUG
...
//=====================================================================
MyVulkan::QueueFamilyIndice MyVulkan::FindQueueFamilies(const VkPhysicalDevice &device)
{
...
//3. logical device
//=====================================================================
void MyVulkan::CreateLogicalDevice()
{
...
//5. swap chain
//=====================================================================
bool MyVulkan::CheckDeviceExtensionSupport(const VkPhysicalDevice &device)
{
	uint32_t extensionCount = 0;
...
//=====================================================================
MyVulkan::SwapChainSupportDetails MyVulkan::QuerySwapChainSupport(const VkPhysicalDevice &device)
{
...
//=====================================================================
VkSurfaceFormatKHR MyVulkan::ChooseSwapSurfaceFormats(const std::vector<VkSurfaceFormatKHR> &availableFormats)
{
	//
...
//=====================================================================
VkPresentModeKHR MyVulkan::ChooseSwapPresentMode(const std::vector<VkPresentModeKHR> &availablePresentModes)
{
...
//choose swap extent
//=====================================================================
VkExtent2D MyVulkan::ChooseSwapExtent(const VkSurfaceCapabilitiesKHR &capabilities)
{
...
void MyVulkan::CreateSwapChain()
{
	...
	//
	uint32_t imageCount = swapChainDetails.capabilities.minImageCount + 1;	//3が理想　2でもok程度の気持ち
	...
	if (indices.graphicsFamily != indices.presentFamily)		//queueが2つ用意できたのならば
	{
...
{
	//cacheが存在する場合には、cacheを読み込んでreturn
	//存在しないor新しいpipeline場合には、新たに作成
	/*
	...
	*/
	...
	//
	auto vertShaderCode = MyVulkan::ReadFile("shaders/objectVert.spv");
	auto fragShaderCode = MyVulkan::ReadFile("shaders/objectFrag.spv");
	//
...
//=====================================================================
//7.1 pipeline to draw coordinates
//=====================================================================
void MyVulkan::CreateGraphicsPipeline(std::string vertPath, std::string fragPath)
{
	uint32_t poipelineSize = mGraphicsPipelines.size();
	mGraphicsPipelines.resize(poipelineSize + 1);
	VkPipeline mGraphicsPipeline;
	//
	auto vertShaderCode = MyVulkan::ReadFile(vertPath);
	auto fragShaderCode = MyVulkan::ReadFile(fragPath);
	//
	VkShaderModule fragShaderModule = CreateShaderModule(fragShaderCode);
	VkShaderModule vertShaderModule = CreateShaderModule(vertShaderCode);
	//
...
void MyVulkan::CreateRenderPass()
{
	...
	colorAttachment.format = mSwapChainImageFormats[0];
...
//=====================================================================
//10. create command pool
//1つのthreadに1つのpoolくらいの気持ちでよい
//=====================================================================
void MyVulkan::CreateCommandPool()
{
...
//=====================================================================
void MyVulkan::CreateCommandBuffers()
{
	...
	for (size_t i = 0; i < mCommandBuffers.size(); i++)
	{
		...
		renderPassInfo.renderArea.extent = mSwapChainExtents[0];
		renderPassInfo.clearValueCount = (uint32_t)clearValues.size();
		renderPassInfo.pClearValues = &clearValues[0];
		vkCmdBeginRenderPass(mCommandBuffers[i], &renderPassInfo, VK_SUBPASS_CONTENTS_INLINE);
		//
		vkCmdBindPipeline(mCommandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, mGraphicsPipelines[0]);
		//vkCmdBindPipeline(mCommandBuffers[3], VK_PIPELINE_BIND_POINT_GRAPHICS, mGraphicsPipelines[1]);
		//
		//bind vertex buffer
		...
		//
		vkCmdBindDescriptorSets(mCommandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, mPipelineLayouts[0], 0, 1,
			&mDescriptorSets[i], 0, nullptr);
		//
		//vkCmdDraw(mCommandBuffers[i], (uint32_t)mVertices.size(), 1, 0, 0);
		vkCmdDrawIndexed(mCommandBuffers[i], (uint32_t)mIndices.size(), 1, 0, 0, 0);
		//
...
//key call back func
//=====================================================================
void MyVulkan::Key(GLFWwindow* window, int key, int scancode, int action, int mods)
{
...
//=====================================================================
void MyVulkan::CleanupSwapChain()
{
	//depth buffer
...
//14. create buffer
//=====================================================================
void MyVulkan::CreateBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties,
...
//=====================================================================
void MyVulkan::CopyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize bufferSize)
{
...
//16. create uniform buffer
//=====================================================================
void MyVulkan::CreateUniformBuffer()
{
	VkDeviceSize bufferSize = sizeof(mUniformBufferObject);
	size_t images = mSwapChainImages.size();
	...
	mUbo.proj = glm::perspective(glm::radians(45.0f),
		mSwapChainExtents[0].width / (float)mSwapChainExtents[0].height, 0.1f, 100.0f);
	mUbo.proj[1][1] *= -1;
...
void MyVulkan::UpdateUniformBuffer(uint32_t currentImage)
{
	...
	//
	void *data;
	//deviceMemory を hostMemoryにmapする、host/device両方からアクセスできる
...
void MyVulkan::CreateDescriptorSets()
{
	...
	allocInfo.descriptorSetCount = (uint32_t)mSwapChainImages.size();
	...
	//
	//20190327 異なるdescriptor setに同じresourceを割り当てているように見える
	for (size_t i = 0; i < mSwapChainImages.size(); i++)
	{
		...
		unsigned u = i % mTextureImageViews.size();
...
//18. create texture image
//=====================================================================
void MyVulkan::CreateTextureImage()
{
...
//18. create image
//=====================================================================
void MyVulkan::CreateImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkFormat format, VkImageTiling tiling,
...
//=====================================================================
void MyVulkan::TransitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout,
	VkImageLayout newLayout, uint32_t mipLevels)
{
	...
	//device : GPU
	//
	//image memory barrier : imageの同期のためのコマンド
	//srcからdstに状態が遷移するまでは、imageにアクセスさせないための措置、コマンド
...
//19. create texture image view
//=====================================================================
void MyVulkan::CreateTextureImageView()
{
...
//=====================================================================
VkFormat MyVulkan::FindSupportedFormat(const std::vector<VkFormat> &candidates, VkImageTiling tiling,
...
void MyVulkan::LoadModel()
{
	...
	//attribに入っている情報をmVertices, mIndices に入れる
...
//22. generate mipmaps
//=====================================================================
void MyVulkan::GenerateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels)
{
